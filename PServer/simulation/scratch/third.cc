...
bool sending = false;
int send_num = 0;
// std::vector<uint32_t> wating_send;
std::vector<Ptr<Node>> wating_send; // 等待队列
...
void ScheduleFlowInputs()
{
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now())
	{
		...
		RdmaClientHelper clientHelper(flow_input.pg, serverAddress[flow_input.src], serverAddress[flow_input.dst], port, flow_input.dport, flow_input.maxPacketCount, has_win ? (global_t == 1 ? maxBdp : pairBdp[n.Get(flow_input.src)][n.Get(flow_input.dst)]) : 0, global_t == 1 ? maxRtt : pairRtt[flow_input.src][flow_input.dst], tag);
...
void permute(vector<int> &nums, vector<bool> &used, vector<vector<int>> &result, vector<int> &path)
{
...
void priority(vector<int> nums, int ori_arr[N])
{
	int k = 4, max_cal_pow = 10, min_time = 99999, max_time = 0, diff = 2;
	...
	int tmp[N] = {0};
	int can_push_time_arr_tmp[N] = {0}, can_push_time_arr[N] = {0};
	int min_time_arr[N] = {0};
	...
	for (int i = 0; i < result.size(); i++)
	{
		...
		{
			tmp[k] += (k + 1) * diff;
			can_push_time_arr_tmp[k] = tmp[k];
		}
		sort(tmp, tmp + N);
		for (int k = 1; k < N; k++)
		{
			int push_diff = diff - (tmp[k] - tmp[k - 1]);
			if (push_diff > 0)
				tmp[k] += push_diff;
		}
		...
		if (tmp[N - 1] < min_time)
		{
			...
			for (int k = 0; k < N; k++)
			{
				min_time_arr[k] = tmp[k];
				ori_arr[k] = result[i][k]; // 原生排序
				can_push_time_arr[k] = can_push_time_arr_tmp[k];
			}
		}
	}
	cout << " Priority order ";
...
void send(Ptr<RdmaQueuePair> q, uint32_t src, uint32_t dst, int send_time)
{
	...
	RdmaClientHelper clientHelper(q->m_pg, serverAddress[src], serverAddress[dst], port, dport, q->m_size, has_win ? (global_t == 1 ? maxBdp : pairBdp[n.Get(src)][n.Get(dst)]) : 0, global_t == 1 ? maxRtt : pairRtt[src][dst], q->m_tg);
...
int main(int argc, char *argv[])
{
	...
	//
	// Assign IP and compute time to each server
	//
	for (uint32_t i = 0; i < node_num; i++)
	{
		if (n.Get(i)->GetNodeType() == 0)
		{ // is server
			...
			// n.Get(i)->setComT(2); //节点的运算时间为2s
			if (i!=0){

				wating_send.push_back(n.Get(i));
