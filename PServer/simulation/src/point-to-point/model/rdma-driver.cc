...
void RdmaDriver::AddQueuePair(uint32_t tg, uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish)
{
	m_rdma->AddQueuePair(tg,size, pg, sip, dip, sport, dport, win, baseRtt, notifyAppFinish);
}
