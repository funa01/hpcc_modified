...
void RdmaHw::AddQueuePair(uint32_t tg,uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	// create qp
...
int RdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch){
	if (ch.l3Prot == 0x11){// UDP
		...
		ReceiveUdp(p, ch);
		//printf("receive Udp at %d in node of %d\n", Simulator::Now(), m_node->GetId());
	}else if (ch.l3Prot == 0xFF){ // CNP
		ReceiveCnp(p, ch);
		//printf("receive CNP at %d in node of %d\n", Simulator::Now(), m_node->GetId());
	}else if (ch.l3Prot == 0xFD){ // NACK
		ReceiveAck(p, ch);
		//printf("receive NACK at %d in node of %d\n", Simulator::Now(), m_node->GetId());
	}else if (ch.l3Prot == 0xFC){ // ACK
		ReceiveAck(p, ch);
		//printf("receive ACK at %d in node of %d\n", Simulator::Now(), m_node->GetId());
	}
	return 0;
}
