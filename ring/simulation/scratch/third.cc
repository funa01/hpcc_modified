...
void ReadFlowInput(){
	if (flow_input.idx < flow_num){
		...
		Ptr<Node> s_node = n.Get(flow_input.src);
		Ptr<Node> d_node = n.Get(flow_input.dst);
		s_node->next_node = d_node;
		}
}
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	...
	if(q->m_tg!=1)
	{
		...
		printf("last sending using time: %lu us *** receive time: %lu us *** send time: %lu us *** node: %d *** tag: %d\n", (Simulator::Now() - q->startTime).GetTimeStep(), Simulator::Now().GetTimeStep(), send_time , n.Get(sid)->GetId(), q->m_tg);
		}
	else
	{
		printf("********************************************************************\n ");
		printf("task finish time: %lu \n", Simulator::Now().GetTimeStep());
	}
}
